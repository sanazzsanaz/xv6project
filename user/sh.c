  ...
  case EXEC:
    ...
    if (strcmp(ecmd->argv[0], "!") == 0)
    {
      ...
      for (int i = 1; ecmd->argv[i]; i++)
        total_len += strlen(ecmd->argv[i]) + 1; // +1 برای فاصله
      ...
      for (int i = 1; ecmd->argv[i]; i++)
      {
        if (mystrstr(ecmd->argv[i], "os"))
        {
          printf("\033[1;34m%s\033[0m ", ecmd->argv[i]); // آبی
        }
        else
        {
